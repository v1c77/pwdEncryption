...
for letter2 in src:
    ...
    #剩下的都是字母,如果是大写字母 且加密后ansi>90说明想或出界，此方法回到开头知道符合为止
        ...
        while letter.isupper() == True and ansi_entire > 90:
            ansi_entire = -26 + ansi_entire

        #如果是大写字母 但是 密文ansi码小于65说明向前出界，那么通过此公式回到结尾，知道不出界为止
        while letter.isupper() == True and ansi_entire < 65:
            ansi_entire = 26 + ansi_entire

        #同理如果原文为小写字母 但是密文ansi码大于122 ，说明向后出界， 通过循环令其归位
        while letter.islower() == True and ansi_entire > 122:
            ansi_entire = -26 + ansi_entire

        #如果原文小写，密文ansi码 小于97 说明向前出界，循环归位
        while letter.islower() ==True and ansi_entire <97:
            ansi_entire = 26 + ansi_entire
